...
process_repo() {
    ...
    if $changes_made; then
        ...
        echo "   🔍 Finding pull request for branch: $BRANCH_NAME..."
        PR_NUMBER=$(gh pr list --head "$BRANCH_NAME" --json number --jq '.[0].number')

        if [ -n "$PR_NUMBER" ]; then
            echo "   💬 Found PR #$PR_NUMBER. Posting comment..."
            gh pr comment "$PR_NUMBER" --body "@coderabbitai pause"
            echo "   ✅ Comment posted."
        else
            echo "   ⚠️ Could not find an open pull request for branch '$BRANCH_NAME'. Skipping comment."
        fi
